...
8. PaintZ updates the existing object's shared `ItemBase` PaintZ state and calls `SetObjectTexture()`.
9. Shared synchronization publishes the finish to clients.
10. CF ModStorage preserves the logical finish assignment through persistence.
...
`tools/paintzgen/paints.json` is the source of truth for finishes. Regenerate outputs with the supplied tooling rather than hand-editing generated files.
