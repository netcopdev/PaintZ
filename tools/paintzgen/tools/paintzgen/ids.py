...
# The type letter is part of the public PaintZ product code.
TYPE_CODES = {
    "solid": "S",
    "camo": "C",
    "pattern": "P",
...
# Common military/color names get useful human-readable suggestions when a
# developer omits `id`. These are suggestions only; explicit IDs are preferred.
COMMON_SUGGESTIONS = {
    "OLIVE DRAB": "ODG",
    "RANGER GREEN": "RGR",
    "NATO GREEN": "NTG",
...
def normalize_suffix(value: str) -> str:
    """Normalize and validate a developer-supplied PaintZ suffix."""
...
def suggest_suffix(name: str) -> str:
    """
    Make a short deterministic, descriptive ID suggestion from a display name.

    This deliberately uses no hash and no registry. It is a convenience for new
    paints; developers should normally commit an explicit `id` after creation.
    """
    ...
    # Existing acronym-like names are already ideal.
    if len(words) == 1 and 2 <= len(words[0]) <= 4 and words[0].isalnum():
        return words[0]

    # 3+ words: acronym is usually the most readable option (FDE/UCP/etc.).
    if len(words) >= 3:
        acronym = "".join(w[0] for w in words if w)[:6]
        return normalize_suffix(acronym if len(acronym) >= 2 else phrase[:3])

    # Two words: first initial + consonant skeleton of the second, e.g. DTN.
    if len(words) == 2:
    ...
    # One ordinary word: first letter + consonants, then vowels if necessary.
