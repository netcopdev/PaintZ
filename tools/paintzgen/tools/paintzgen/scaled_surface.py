...
def scale_pattern_fill(
    path: Path,
    size: tuple[int, int],
    pattern_scale: float,
) -> Image.Image:
    """
    Return the normal PaintZ pattern fill with its geometry scaled.

    pattern_scale > 1 enlarges the pattern geometry (zooms in).
    pattern_scale < 1 reduces the geometry and repeats it to fill the canvas.
    """
    if pattern_scale <= 0:
...
def render_surface_scaled(
    ...
    """
    Render a PaintZ surface while scaling only pattern geometry.

    Solid finishes use the normal render path. Wear/grime/scratch overlays are
    applied after the pattern transform so their geometry is not scaled with
    the camouflage.
    """
